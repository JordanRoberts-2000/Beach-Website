...
type Props = {
    ...
    password: boolean,
    submitEmpty: boolean,
    emtpySetter: Dispatch<SetStateAction<boolean>> | null
}
...
const TextInput = ({refValue, validationFunc, title, maxLength, debounceDuration, tooltip, tooltipMessage, password, submitEmpty, emtpySetter}:Props) => {
    ...
    const onChangeFunction = (name:string) => {
        if(emtpySetter !== null)resetError(submitEmpty, emtpySetter)
    ...
    return (
        <div className='w-[85%] px-1 flex mx-auto flex-col'>
            <div className='flex'>
                ...
                <div className='mt-auto mr-1 ml-auto relative flex cursor-pointer group'>
                    ...
                    <>
                        <span className='absolute top-[50%] opacity-0 duration-150 group-hover:opacity-[1] pointer-events-none translate-y-[-50%] left-[100%] 
                             w-[300px] z-20 bg-transparent text-xs font-medium text-gray-600 p-2 ml-2 rounded-md'>
                            {tooltipMessage}
                        </span>
                        <svg className='h-3 fill-black mt-auto mb-1' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM216 336h24V272H216c-13.3 0-24-10.7-24-24s10.7-24 24-24h48c13.3 0 24 10.7 24 24v88h8c13.3 0 24 10.7 24 24s-10.7 24-24 24H216c-13.3 0-24-10.7-24-24s10.7-24 24-24zm40-208a32 32 0 1 1 0 64 32 32 0 1 1 0-64z"/></svg>
                    </>}
                </div>
            </div>
            <div className='flex relative flex-1'>
                <input ref={refValue} id={title} type={password ? 'password' : 'text'} 
                       className={`outline-none taxt-sm font-medium text-gray-700 bo py-1 pl-2 flex-1 pr-2 border border-black bg-gray-100 rounded-sm`} 
                ...
                <div className='absolute top-[50%] translate-y-[-50%] right-0 flex flex-col w-4 h-4 mr-2 justify-center items-center overflow-hidden'>
                    <motion.div variants={varients} animate={(textInputError || submitEmpty) ? 'errorActive1' : 'inActive1'} className={`h-[2px] bg-black`}></motion.div>
                    <motion.div variants={varients} animate={(textInputError || submitEmpty) ? 'errorActive2' : 'inActive2'} className={`h-[2px] bg-black`}></motion.div>
                        <div className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">
                            <svg className="w-5 h-5 stroke-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2}>
            ...
            <span className={`${(textInputError || submitEmpty) ? 'opacity-1' : 'opacity-0'} z-20 duration-200 whitespace-nowrap font-semibold text-xs mb-[3px] mr-2 text-red-500`}>
